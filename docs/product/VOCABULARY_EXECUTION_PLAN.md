...
Tình trạng hiện tại: **30/56 task xong**, cộng `VOC-API-01` ở trạng thái *code xong, chưa verify trên project thật* (thêm 3 task ở trạng thái *adapter viết xong, chưa verify trên database*: `VOC-API-02s/03s/05s`). `VOC-PLAN-05` đã chốt theo ADR-004, kéo theo 4 task mới: `VOC-INFRA-09` (job dọn retention) và `VOC-WEB-11` (copy cảnh báo tiến độ cục bộ, chờ Product chốt câu chữ); `VOC-WEB-09/10` đã xong. Toàn bộ lát cắt dọc (app, API, UI, a11y) đã chạy trên fixture. Việc còn lại: quyết định Product (`VOC-PLAN-02/03/07/08`), hạ tầng cần credential hoặc quyền admin (`VOC-DATA-07a/07b`, `VOC-INFRA-03/06/07`), adapter Supabase (`VOC-API-01/02s/03s/05s`), và QA (`VOC-QA-02/04/05/06/07/08`).
...
| ~~VOC-PLAN-02~~ | ~~Chốt beta deck~~ | VOC-PLAN-01 | **Decided 2026-08-21:** beta list là General Academic, Environment, Technology, Education (đúng khuyến nghị spec §13). Danh sách sống trong `BETA_DECKS` ở `scripts/vocabulary/publish-beta-decks.mjs`. |
...
| ~~VOC-PLAN-05~~ | ~~Chốt guest/account policy~~ | VOC-PLAN-01 | **Decided (ADR-004):** giữ D-12; retention 3 tháng không hoạt động (cascade xoá state); **không** mời tạo tài khoản, chỉ link ở footer; đổi thiết bị khôi phục bằng link OAuth identity (`enable_manual_linking = true`); rate limit anonymous 50/giờ/IP; nút tự xoá dữ liệu theo `auth.uid()`. |
...
| ~~VOC-INFRA-06~~ | ~~Bật Supabase Anonymous Auth~~ | VOC-PLAN-05 | **Done and verified 2026-08-21:** `supabase/config.toml` nay đặt `enable_anonymous_sign_ins = true`, `enable_manual_linking = true`, `anonymous_users = 50` theo ADR-004. Production đã được xác nhận qua Management API: `external_anonymous_users_enabled=true`, `security_manual_linking_enabled=true`, `rate_limit_anonymous_users=50`. Anonymous sign-in đã chạy thật qua app. |
| VOC-INFRA-09 | Job dọn anonymous user quá hạn | VOC-PLAN-05 | ADR-004 định retention 3 tháng nhưng Supabase **không** tự xoá anonymous user. DoD: scheduled job xoá user không hoạt động >3 tháng, verify cascade dọn sạch `learner_card_states`/`learner_card_reviews`, và log số dòng đã xoá. Không có job này thì retention chỉ tồn tại trên giấy. |
...
| ~~VOC-DATA-07a~~ | ~~Seed dev/staging content~~ | VOC-DATA-03, VOC-DATA-04 | **Applied and verified 2026-08-21:** 5.275 card / 23 deck / 8.271 membership, một `source_version` duy nhất khớp catalog SHA `c6b29ad4`, 7.309 nghĩa. Toàn bộ `content_status`/`publish_status` = `draft`. RLS verify thật: anon query trả **0 dòng** dù dữ liệu tồn tại, `learner_card_states` trả 401 cho anon. **Lưu ý:** vì mọi thứ còn `draft`, learner **chưa thấy gì** — dòng "đủ để API/UI query thật" trong bản kế hoạch cũ là sai, cần `VOC-DATA-07b` publish ít nhất một deck. |
| ~~VOC-DATA-07b~~ | ~~Publish beta content~~ | VOC-DATA-07a, VOC-PLAN-02 | **Applied and verified 2026-08-21:** `general_academic` 622, `environment` 507, `technology` 341, `education` 265 — 4 deck + 1.642 card sang `published`, 0 thẻ sót. RLS verify: anon chỉ thấy đúng 4 deck; 13 deck `draft` vẫn kín dù chứa thẻ dùng chung đã published. Script `vocab:publish-beta` idempotent và có `--unpublish` để rollback. |
...
| ID | Task | Dependency | Definition of done |
|---|---|---|---|
| ~~VOC-API-01~~ | ~~Identity resolver guest/user~~ | VOC-DATA-02, VOC-INFRA-06 | **Verified on live project 2026-08-21:** Anonymous Auth cấp phiên thật, cookie httpOnly, learner giữ nguyên qua restart. `auth/anonymous.ts` gọi GoTrue REST cấp UUID ngay lần vào đầu; `identity.ts` tái dùng access token còn hạn, refresh khi hết hạn (skew 60s), refresh hỏng thì cấp learner mới theo ADR-004. Token nằm trong cookie httpOnly, refresh cookie 90 ngày khớp retention. **Còn lại:** bật anonymous trên dashboard remote (`VOC-INFRA-06`) và seed content (`VOC-DATA-07a`) thì mới chạy end-to-end được. |
...
| ~~VOC-API-02s~~ | ~~Deck catalog trên Supabase~~ | VOC-DATA-07a | **Verified on live data 2026-08-21:** trả đúng 4 deck beta với count thật. `repository.supabase.ts` query `vocabulary_decks` + join `vocabulary_deck_cards`, chỉ `select` cột learner-facing (không `select=*`), không trả corpus. Còn chờ `VOC-DATA-07a` seed và `VOC-API-01` cấp token để chạy được. |
...
| ~~VOC-API-03s~~ | ~~Queue trên Supabase~~ | VOC-API-01, VOC-DATA-07a | **Verified on live data 2026-08-21:** queue `new` trả thẻ thật, thẻ đã chấm rời queue. thứ tự và limit vẫn do `service.ts` quyết (không đổi khi đổi data source); adapter chỉ cấp card + learner state. Chờ seed + token. |
...
| ~~VOC-API-05s~~ | ~~Review write transactional/bền~~ | VOC-API-03, VOC-API-04 | **Verified on live data 2026-08-21:** submit + replay idempotent, tiến độ sống sót qua restart server (VOC-07). Dùng RPC deployed sẵn, không phải migration mới: RPC `submit_vocabulary_review` (`security invoker`) insert event + upsert state trong một transaction; idempotency do unique `(learner_id, idempotency_key)` quyết chứ không phải read-then-write, nên retry double-tap không race. Migration `20260821090000` **chưa chạy**. Transaction/RLS thật do `VOC-QA-02` verify. |
...
| ~~VOC-WEB-09~~ | ~~Link tạo tài khoản ở footer~~ | VOC-PLAN-05 | **Implemented:** footer ở `app/vocabulary/layout.tsx` nên có mặt trên mọi màn Vocabulary; link thường, không banner/modal/gate. Trỏ tới `/vocabulary/account`. **Giới hạn phải biết:** trang đó nói rõ liên kết tài khoản **chưa khả dụng** — `VOC-API-01` mới làm anonymous session, chưa có luồng OAuth linking. Cần một task riêng cho luồng đăng nhập/link identity. |
| ~~VOC-WEB-10~~ | ~~Nút xoá dữ liệu học~~ | VOC-PLAN-05 | **Implemented and verified on live data:** RPC `delete_my_vocabulary_data()` (migration `20260821120000`, đã apply) xoá state + review trong một transaction theo `auth.uid()`. Hàm **không nhận tham số nào**, nên về mặt cấu trúc không thể trỏ vào dữ liệu người khác. Nút chỉ hiện ở dashboard, không hiện trong phiên ôn. Có bước xác nhận nói rõ mất gì và không khôi phục được. Integration test xác nhận xoá của Alice không đụng dòng của Bob. |
| VOC-WEB-11 | Copy cảnh báo tiến độ cục bộ | VOC-PLAN-05 | ADR-004 ghi nhận rủi ro: người học không biết tiến độ chỉ nằm trên thiết bị này và sẽ mất khi đổi máy. DoD: chỗ hiển thị tiến độ nói rõ điều đó và trỏ tới cách giữ lại. Copy chưa được spec — cần Product chốt. |
...
| ~~VOC-QA-02~~ | ~~Integration test review write~~ | VOC-API-05s | **Implemented and passing on the live project 2026-08-21:** 8 test trong `test/vocabulary/review-write.integration.test.mjs` — một rating ghi đúng một event + một state; replay chỉ persist một event và không tăng `review_count`; hai rating đồng thời thì đúng một cái thắng, cái thua trả `StaleLearnerStateError` và **không để lại event mồ côi** (chứng minh transaction); RLS chặn learner đọc dòng của learner khác kể cả khi truyền id người khác; state đọc lại được từ client mới (VOC-07); card chưa publish không chấm được. Chạy bằng `npm run vocab:test-integration` (script tự set flag nên chạy được cả PowerShell lẫn bash); `npm test` thường vẫn offline vì 8 test này skip khi không có flag. Test tự xoá mọi anonymous learner nó tạo. |
...
| VOC-QA-04 | E2E core journeys | VOC-WEB-07 | **Suite viết xong nhưng CHƯA chạy được — không tính là đạt.** `tests/e2e/vocabulary-journeys.spec.mjs` (Playwright, viewport 360px, chạy trên fixture nên không cần credential và không ghi vào project thật) phủ 11 hành trình gồm đủ 8 mục DoD. **Chặn:** sandbox của agent trả 403 cho `/_next/static/chunks/*` với mọi trình duyệt do automation khởi chạy (Chromium bundled, Chrome, Edge — đã thử cả `--no-proxy-server`), nên trang không bao giờ hydrate; `next start` cũng hỏng trong môi trường này. Selector chưa được kiểm chứng lần nào. **Việc còn lại:** chạy `npm run test:e2e` trên máy dev thật và sửa selector nếu lệch. |
...
| ~~VOC-QA-06~~ | ~~Performance check~~ | VOC-DATA-07a, VOC-WEB-08 | **Measured and fixed 2026-08-21.** Đo trên corpus thật (4 deck / 1.642 card published): first load gửi **660B** deck summary + **746B** progress — không có card nào. Phát hiện: `getDeckCatalog` kéo **1.735 card** về chỉ để `.length`, làm `/decks` mất 2,5–3,8s, sát/vượt ngưỡng 3s. Sửa bằng cách dùng view `vocabulary_deck_summary` (count trong DB, RLS còn nguyên) và chỉ map deck cho **các thẻ learner đã chấm**. Kết quả: decks **0,39–0,41s** (nhanh ~7×), progress **0,58–0,65s** (~4×). Chi phí catalog giờ tỉ lệ với tiến độ người học, không phải kích thước corpus. |
| ~~VOC-QA-08~~ | ~~Viết rollback/backup runbook~~ | VOC-DATA-07b, VOC-INFRA-03 | **Written:** `docs/runbooks/vocabulary-rollback-and-backup.md` — rút content về draft, khôi phục content version trước (kèm bẫy upsert không xoá thẻ thừa), rollback migration (kèm cảnh báo **không** được drop `submit_vocabulary_review` và hai thuộc tính phải giữ), dữ liệu learner, artifact audio. Lệnh trong runbook đã chạy thử. Ghi rõ 4 gap thật: chưa có backup audio, một project cho mọi môi trường, retention chưa enforce, chưa xác nhận cửa sổ PITR. |
...
**Cập nhật (2026-08-21):** seam data đã tách — `repository.ts` là interface, `service.ts` chỉ nói chuyện với interface đó, `repository.factory.ts` chọn adapter theo `VOCABULARY_DATA_SOURCE` (mặc định `fixture`). Adapter Supabase và RPC transactional đã viết. **Chưa ghi gì vào database**: chưa seed, chưa apply migration mới. Blocker duy nhất còn lại để bật `supabase` là access token của learner — tức `VOC-API-01`, phụ thuộc `VOC-INFRA-06`/`VOC-PLAN-05`.
