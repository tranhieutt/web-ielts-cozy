...
| ~~VOC-INFRA-08~~ | ~~Bootstrap application framework~~ | VOC-PLAN-01 | **Implemented:** npm workspace `apps/web` chạy Next.js App Router theo D-10; `npm run web:dev`/`web:build`, typecheck sạch, CI build + typecheck app. App đã nhận toàn domain: route Next thật thắng filesystem, các route prototype còn lại được rewrite về `prototype.html` đồng bộ ở build time; `verify-static-runtime` kiểm mọi route claim bởi app có page và mọi route prototype có handler. |
...
| ID | Task | Dependency | Definition of done |
|---|---|---|---|
| ~~VOC-WEB-01~~ | ~~Foundation route `/vocabulary`~~ | VOC-API-02, VOC-INFRA-08 | **Implemented:** route Next.js App Router, không phụ thuộc state của mockup tĩnh. Shared `SiteNav` nay đã giao: giữ 11 destination/label/order của prototype, đánh dấu Từ vựng active cho cả catalog/review, scroll ngang thay vì wrap ở mobile, và có skip link. |
...
**Đã chạy xong (2026-08-20).** Bước 1 dựng trên fixture (`VOC-INFRA-08` + `VOC-API-02/03/05` + route `/vocabulary`). Bước 2 đổi đường *đọc* sang Supabase thật (`VOC-DATA-07a/07b`, `VOC-API-02s/03s`). Bước 3 cũng đã đóng: `VOC-API-01` cấp/link identity Supabase thật và `VOC-API-05s` ghi review/state transactionally; reload và server restart vẫn giữ tiến độ. Fixture không còn nằm trên production write path.
...
`VOC-PLAN-05` (guest/account policy) **đã chốt** ở ADR-004/D-20; chuỗi `VOC-INFRA-06` → `VOC-API-01` → `VOC-API-05s` → `VOC-QA-02` đã hoàn tất và verify trên project thật, gồm Google link giữ nguyên anonymous UUID. Ràng buộc OAuth còn lại chỉ thuộc checklist deploy production, không còn chặn vertical slice.
