...
begin
  ...
  if v_written is null then
    -- PT409, deliberately NOT 40001. PostgREST treats serialization_failure as
    -- transient and retries it automatically, which turned a lost race into an
    -- unbounded retry loop that wedged the connection. PTxxx sets the HTTP
    -- status directly, so this returns 409 Conflict once and the caller decides.
