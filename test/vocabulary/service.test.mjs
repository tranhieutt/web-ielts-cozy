...
import {
  ...
  resetFixtureState,
} from '../../apps/web/src/features/vocabulary/repository.fixture.ts';
import {
  buildReviewQueue,
  getDeckCatalog,
  getLearnerProgress,
...
/**
 * Every test drives the service through the seam, exactly as a route handler
 * does, so these stay honest when the Supabase adapter is the one plugged in.
 */
...
test('deck catalog reports publishable counts without shipping the corpus', async () => {
  const [deck] = await getDeckCatalog(repo, LEARNER);
...
test('new mode returns unseen cards easiest-CEFR first and respects the limit', async () => {
  const cards = await buildReviewQueue(repo, LEARNER, { deck: 'environment', mode: 'new', limit: 5 });
...
test('a rated card leaves the new queue and comes back only when due', async () => {
  const [first] = await buildReviewQueue(repo, LEARNER, { deck: 'environment', mode: 'new', limit: 1 });

  const past = new Date(Date.now() - 60 * 60 * 1000);
  await submitReview(repo, LEARNER, { cardId: first.id, rating: 'again', idempotencyKey: KEY(), reviewedAt: past });

  const stillNew = await buildReviewQueue(repo, LEARNER, { deck: 'environment', mode: 'new', limit: 20 });
  assert.ok(!stillNew.some((card) => card.id === first.id), 'seen card must leave the new queue');

  const due = await buildReviewQueue(repo, LEARNER, { deck: 'environment', mode: 'due', limit: 20 });
  assert.deepEqual(due.map((card) => card.id), [first.id]);
});

test('a card scheduled into the future is not in the due queue', async () => {
  const [first] = await buildReviewQueue(repo, LEARNER, { deck: 'environment', mode: 'new', limit: 1 });
  await submitReview(repo, LEARNER, { cardId: first.id, rating: 'known', idempotencyKey: KEY() });
...
test('replaying an idempotency key returns the first result, not a second stage', async () => {
  const [card] = await buildReviewQueue(repo, LEARNER, { deck: 'environment', mode: 'new', limit: 1 });
  const key = KEY();

  const first = await submitReview(repo, LEARNER, { cardId: card.id, rating: 'known', idempotencyKey: key });
  const replay = await submitReview(repo, LEARNER, { cardId: card.id, rating: 'known', idempotencyKey: key });
  ...
  const advanced = await submitReview(repo, LEARNER, { cardId: card.id, rating: 'known', idempotencyKey: KEY() });
...
test('learner progress does not leak between learners', async () => {
  const [card] = await buildReviewQueue(repo, LEARNER, { deck: 'environment', mode: 'new', limit: 1 });
  await submitReview(repo, LEARNER, { cardId: card.id, rating: 'known', idempotencyKey: KEY() });

  const [otherDeck] = await getDeckCatalog(repo, OTHER_LEARNER);
  assert.equal(otherDeck.progress.newCount, 20);
  assert.equal(otherDeck.progress.learningCount, 0);

  const otherQueue = await buildReviewQueue(repo, OTHER_LEARNER, { deck: 'environment', mode: 'new', limit: 20 });
...
test('unknown deck and unknown card are rejected rather than guessed', async () => {
  assert.deepEqual(await buildReviewQueue(repo, LEARNER, { deck: 'no-such-deck', mode: 'new', limit: 5 }), []);
  await assert.rejects(
    () => submitReview(repo, LEARNER, { cardId: 'w_missing', rating: 'known', idempotencyKey: KEY() }),
    /unknown card/,
  );
});

test('learner-facing payload carries no Chinese source text or Youdao audio', async () => {
  const cards = await buildReviewQueue(repo, LEARNER, { deck: 'environment', mode: 'new', limit: 20 });
...
test('learner progress separates due from scheduled and never inflates mastery', async () => {
  const cards = await buildReviewQueue(repo, LEARNER, { deck: 'environment', mode: 'new', limit: 3 });
  const past = new Date(Date.now() - 60 * 60 * 1000);

  // One card rated in the past -> due again now; two rated now -> scheduled.
  await submitReview(repo, LEARNER, { cardId: cards[0].id, rating: 'again', idempotencyKey: KEY(), reviewedAt: past });
  await submitReview(repo, LEARNER, { cardId: cards[1].id, rating: 'known', idempotencyKey: KEY() });
  await submitReview(repo, LEARNER, { cardId: cards[2].id, rating: 'known', idempotencyKey: KEY() });

  const progress = await getLearnerProgress(repo, LEARNER);
...
test('progress is empty for a learner who has rated nothing', async () => {
  const progress = await getLearnerProgress(repo, OTHER_LEARNER);
